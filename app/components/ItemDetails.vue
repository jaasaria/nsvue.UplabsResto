<template>
	<Page actionBarHidden="true" class="anim-page" backgroundSpanUnderStatusBar="true" @loaded="onLoaded">
  <StackLayout class="main" verticalAlignment="top" >

		<GridLayout class="anim-cover" rows="auto" columns="*">
				<Image row="0" col="0" marginTop="-40" height="180" stretch="aspectFill" class="card-img"
				:src="item.cover" />
				<Label row="0" col="0" verticalAlignment="top" horizontalAlignment="left"  @tap="close" :text="'fa-arrow-left' | fonticon" class="fa close"/>
		</GridLayout>
		...
		<GridLayout rows="auto,auto,auto,auto" columns="auto" class="content">
			...
			<GridLayout class="anim-itemInfo" marginTop="15" row="1" width="100%" columns="auto,*" rows="auto,auto,auto,auto" verticalAlignment="center">
				<Label :text="categoryIcon | fonticon" row="0" col="0" rowSpan="2" :backgroundColor="item.categoryTag" class="fa category-icon"/>
				<Label row="0" col="1" class="item-name" textwrap="true" verticalAlignment="bottom" horizontalAlignment="left" 
					:text="item.name" />
			...
			<StackLayout class="line anim-likes" row="2" width="100%"  marginTop="10"/>

			<GridLayout class="anim-likes" marginTop="5" width="100%" row="3" columns="auto,*,auto,auto" rows="auto">
				<GridLayout col="0" rows="auto" columns="auto,auto"  @tap="toggleLike"> 
					<Label col="0" row="0" ref="like" class="like-icon fa" :class="[isLike ? 'liked-active' : 'default']"	 :text="isLike ? 'fa-thumbs-up':'fa-thumbs-o-up' | fonticon"/>
				...
				<GridLayout col="2" rows="auto" columns="auto,auto"  @tap="toggleHeart" marginRight="15"> 
					<Label col="0" row="0" ref="favorite" class="like-icon  fa" :class="[isHeart ? 'heart-active' : 'default']"	 :text="isHeart ? 'fa-heart':'fa-heart-o' | fonticon"/>
		...
		<Gridlayout rows="auto,*" class="content anim-content" marginTop="15">
			...
			<GridLayout row="0" rows="auto" marginBottom="5" columns="auto, auto">
				...
				<Label col="1" class="description-text" text="Description" textWrap="true" />
			</GridLayout>
	...
	export default {
		...
		methods: {
			...
			animateFrom(){
				...
				images.opacity = 0
				images.scaleX = 0; cover.scaleY = 0

				cover.translateY = 200
				cover.opacity = 0
				cover.scaleX = 0; cover.scaleY = 0

				page.backgroundColor = "#d4d6d8",
			...
			animateLike(){
				...
				imgLogo.animate({ scale: { x: 0.6, y: 0.6}, duration: 100, delay: 0 })
					.then(function () { return imgLogo.animate({ scale: { x: 1.2, y: 1.2, duration: 50 } }); })
					.then(function () { return imgLogo.animate({ scale: { x: 1, y: 1, duration: 100 } }); })
					.then(function () {
				})
			},
			animateFavorite(){
				let imgLogo = this.$refs.favorite.nativeView
				imgLogo.animate({ scale: { x: 0.6, y: 0.6}, duration: 50, delay: 0 })
					.then(function () { return imgLogo.animate({ scale: { x: 1.2, y: 1.2, duration: 50 } }); })
					.then(function () { return imgLogo.animate({ scale: { x: 1, y: 1, duration: 100 } }); })
					.then(function () {
				})
